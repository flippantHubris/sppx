...
const getUrl = url =>
  `https://api.sppx.io/system/files${url.replace('private:/', '')}`;

const getUrl2 = url =>
  `https://api.sppx.io/sites/default/files${url.replace('public:/', '')}`;

const toggleWith = field => (val, key, obj) => field === key;

export default (state = INTIT_INVEST, action) => {
  switch (action.type) {
    ...
    case 'TOGGLE2':
      if (action.object === 'investor') {
        ...
        return {
          ...state,
          accreditation: R.mapObjIndexed(
            toggleWith(action.field),
            state.accreditation
          )
        };
      } 
