...
import { Header, Card as MyCard } from '../components/common';
import { Image, TouchableOpacity } from '../globals';
...
const Details = ({ details }) => {
  ...
  return <View>{details.map(detail => <TextRow key={detail.title} detail={detail} />)}</View>;
};

const TextRow = ({ detail }) => {
  let { value, title } = detail;
...
const Files = ({ files }) => {
  ...
  return <View>{filesArray.map(file => <FileRow key={file.title} file={file} />)}</View>;
};
...
export default connect(mapStateToProps, { getIssueDetail, showBackButton })(InvestDetailScreen);
